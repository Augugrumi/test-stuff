...
def send_thread(sock_dst, options, pkt, sock_src):
    while True:
        data = sock_dst.recvfrom(65507)
        while len(data) > 0:
            #if not options.reverse:
            data = raw(pkt) + data
            sock_src.sendto(data[:2048])
            data = data[2048:]
...
def recv():
    ...
    Thread(target=send_thread, args=[
           sock_dst, options, pkt, sock_src]).start()
    i = 0
    while True:
        ...
        while len(data) > 0:
            #if not options.reverse:
            data = raw(pkt) + data
            sock_dst.sendto(data[:2048], dst_addr)
            data = data[2048:]
            time.sleep(0.05)
